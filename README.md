...
`scripts/collect.py`は1リクエストで最大40記事をまとめて分類するため、通常運用
（1日1回のcron実行）では数リクエストで収まる想定だが、同日中に手動実行を
繰り返す（動作確認等）と上限に達し、その日は分類が全て「その他」・
ハッシュタグなしにフォールバックする。上限に達した状態で記事が収集されると
`seen_urls.json`に既収集として記録されるため、後から再実行しても再分類されない。
正しく分類させ直したい場合は、該当日の`data/YYYY-MM-DD.json`と`data/seen_urls.json`
から該当分を削除してから再実行する。
